...
By default it reads from `/Volumes/principal_lab_db/landing/operational_data`. The location can be overridden using the `raw_path` pipeline configuration when the pipeline is created.
The target catalog and schema can also be set using the `catalog` and `schema` configurations. This allows the same pipeline to be deployed in different environments (for example DEV vs. PROD) just by changing the configuration values.
